...
// Handle Multiple Image Upload for Upcoming Projects
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['upload_project'])) {
    ...
    if (!empty($_FILES["images"]["tmp_name"][0])) {
        ...
        if (!empty($uploaded_images)) {
            ...
            $stmt = $conn->prepare("INSERT INTO upcoming_projects (image_url, client_name, area, location) VALUES (?, ?, ?, ?)");
            $stmt->bind_param("ssss", $image_urls, $client_name, $area, $location);

            if ($stmt->execute()) {
                $_SESSION['message'] = "Upcoming project uploaded successfully!";
            } else {
...
$query = "SELECT * FROM upcoming_projects ORDER BY created_at DESC";
...
<a href="admin_dashboard.php" class="back-button">⬅ Back</a>
    ...
    <h2 class="text-center fade-in">Upload Upcoming Project</h2>
    ...
    <h2 class="mt-5 text-center fade-in">Upcoming Projects</h2>
